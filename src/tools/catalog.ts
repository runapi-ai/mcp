...
export function registerCatalogTools(server: McpServer, dependencies: DiscoveryToolDependencies) {
  ...
  server.tool(
    ...
    async ({ model, service, action }) => {
      return jsonText(await getModelInfoHandler({ model, service, action }, dependencies.contract, dependencies.client));
    }
  );
  ...
  server.tool(
    ...
    async ({ service, action, model }) => {
      return jsonText(await checkPricingHandler({ service, action, model }, dependencies.contract, dependencies.client));
    }
  );
