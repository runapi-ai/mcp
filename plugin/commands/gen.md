...
5. Generate one new opaque idempotency key for this logical task.
6. Delegate the creation to the `task-executor` agent with exact service, action, model, params, idempotency key, and wait settings.
...
Do not retry an unknown create result automatically. An explicit retry must reuse the original idempotency key and identical input.
